...
contract PayrollEscrow {
    ...
    uint256 constant SEVERANCE_DAYS = 15;
    ...
    function depositToEscrow() external payable {
        require(companies[msg.sender].hrWallet != address(0), "Company not registered");
    ...
    function initiateTermination(address _employee) external {
        ...
        require(
            msg.sender == emp.employeeAddress || 
            companies[msg.sender].hrWallet == msg.sender,
            "Not authorized"
        );
    ...
    function paySalary(address _employee) external {
        ...
        uint256 timeSinceLastPayment = block.timestamp - emp.lastPaymentDate;
        require(timeSinceLastPayment >= 30 days, "Payment not due yet");
